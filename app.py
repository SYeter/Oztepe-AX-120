...
class AppState:
    ...
    yolluk_min_size_ratio: float = 3.0
...
class MainWindow(QWidget):
    ...
    def apply_inputs(self) -> None:
        ...
        self.state.yolluk_min_size_ratio = max(0.1, min(50.0, yolluk_ratio))
    ...
    def update_frame(self) -> None:
        ...
        if not rois_selected:
            ...
            if signal == 0:
                ...
                self.update_status(
                    f"⏱️ Zaman aşımı sayacı çalışıyor ({timeout_elapsed:.1f}/{self.state.timeout_seconds} sn)."
                )

                if timeout_elapsed >= self.state.timeout_seconds:
